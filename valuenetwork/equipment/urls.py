...
urlpatterns = patterns("",
    url(r"^log-equipment-use/(?P<equip_resource_id>\d+)/(?P<context_agent_id>\d+)/(?P<pattern_id>\d+)/(?P<equip_svc_rt_id>\d+)/(?P<equip_fee_rt_id>\d+)/(?P<tech_rt_id>\d+)/(?P<consumable_rt_id>\d+)/(?P<payment_rt_id>\d+)/(?P<ve_id>\d+)/$",
